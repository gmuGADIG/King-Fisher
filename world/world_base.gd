...
func _ready() -> void:
	...
	if multiplayer.is_server():
		...
		var new_player_skin_index = randi_range(0,CharacterSelect.character_textures.size()-1)
		spawn_player.rpc(1, $Players.get_safe_spawn_point(),new_player_skin_index)
...
func on_player_join(id : int) -> void:
	...
	for child in get_children():
		if child is Player:
			var p: Player = child
			var skin_index : int = Multiplayer.player_list[p.get_multiplayer_authority()].character_texture_id
			spawn_player.rpc_id(id, p.get_multiplayer_authority(), p.position, skin_index)
	
	var new_player_skin_index = randi_range(0,CharacterSelect.character_textures.size()-1)
	spawn_player.rpc(id,$Players.get_safe_spawn_point(),new_player_skin_index)
	# Sends information about new players to everyone
	Multiplayer.broadcast_player_info()

@rpc("reliable", "call_local")
func spawn_player(id: int, pos: Vector3, texture_id : int) -> void:
	Debug.log("Creating player of id ",id)
	var new_player: Player = player.instantiate()
	
	new_player.position = pos
	##Set skin
	(new_player.player_mesh.mesh.material_override as StandardMaterial3D).albedo_texture = CharacterSelect.character_textures[texture_id]
	...
	if multiplayer.is_server():
		...
		if server_conn != null: # should only be == null if we started this level w/ F6
			...
			server_conn.character_texture_id = texture_id
