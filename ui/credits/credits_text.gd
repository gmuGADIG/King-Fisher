...
func to_letter_rigid() -> void:
	...
	for i in text.length():
		...
		var letter_collider : CircleShape2D = CircleShape2D.new()
		letter_collider.radius = font_size*0.25
		new_rigid.collision_shape.shape = letter_collider
